...
- **[P9, §2.2]'s mean of `(−1)^Ω` over rough `n`.** The `4×10⁷` factorisations are inside
  ...
  not check that row.
| `verify_paired_almost_primes.py` | [P12, Prop. 1], and through it [P12, Thm 1] and [P12, Thm 2] | encloses the dimension-two DHR pair from `σ₂` on `(0,4]`, the integral recursion to `5.360`, and a monotone march of the two delayed equations above `α₂`; then returns a lower bound for `C_r(θ;w,u) = (r+1−u) f₂(θw) − 2∫(1/t − u) F₂(w(θ−t)) dt`. `α₂` is bracketed in `[5.356,5.360]` and handled by the fact that `s²F₂(s)` increases throughout, so no decimal place of it is relied on | grid step `0.0005` to `s = 10`; the integral on 6,000 pieces | exact enclosure — outward-rounded Decimal at 34 digits, monotone Riemann bounds, no quadrature rule. Three of the four bounds carry a wide margin; the `theta = 0.514` row is deliberately near its threshold, at about ten enclosure widths |
