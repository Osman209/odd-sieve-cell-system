...
**P/M — [P6, §2.9] (a proved lower bound on the survivor count).** Writing $m(d)$ for the number of lines striking a cell and $S_i = \sum_d \binom{m(d)}{i}$, the odd Bonferroni truncation $C_M \ge S_0 - S_1 + \cdots - S_{2\ell+1}$ holds for every $\ell$ [**P**], and is **exact** at an odd order if and only if that order reaches $\max_d m(d)$ [**P**], the truncation error being $\sum_{d : m(d) \gt j}\binom{m(d)-1}{j}$. This is the first proved lower bound on $C_M$ in the series; every earlier value was a direct count. Measured [**M**]: order three is exact to $M = 21$, order five to $141$, order seven to $381$; and $\max_d m(d) = 9, 10, 11, 12, 13$ at $M = 10^3, 10^4, 5\cdot10^4, 2\cdot10^5, 10^6$. The order at which the truncation becomes exact is $\max_d m(d)$, and that is $\Theta(\log M/\log\log M)$ [**P**] by [P6, Prop 1'] — small against $\pi(10^6) = 78{,}498$ lines, but **not** of the order of the mean multiplicity, which is $\approx 2\log\log M$; an earlier version inferred the second from the first and the inference is invalid. **What it does not buy:** a theorem needs uniform control of $S_1,\dots,S_j$ for $j \asymp \log\log M$, sums over $j$-tuples of primes of the shape that makes $S_1$ diverge.
...
- **[P9, Thm 5] (overlap as a resource).** Splitting the pairs of future lines into those piling on one member ($A$) and those splitting across the cell ($B$) gives the unconditional bound $T \ge C-R+\tfrac23A+\tfrac19B$. That pair is one of the two extreme points of the complete admissible coefficient region [P9, Thm 5']; the other, $B-\tfrac23A$, is better whenever $A \lt 2B/3$ and returns the exact identity $T = C-R+S$ at the cubic cut, so the bound and the identity are the two ends of one family. It is the one place in this work where the estimates needed run the other way — an upper bound for $R$, lower bounds for $A$ and $B$ — and the free model puts the critical cut exponent at $0.50681$ against $0.60653$ for $R\lt C$ alone. Evaluating the same bound on the true distribution of begun lines instead — the model overstates the pair term badly and the strike term by a constant $0.93$ — puts the predicted threshold at $0.5385$ **under that model** [**M**], with direct measurement at $X = 2\cdot10^8$, $2\cdot10^9$ and $2\cdot10^{10}$ approaching it from below at the expected rate; it is a model prediction and not a proved threshold.
...
- **[P9, Thm 7].** The classical Buchstab upper bound for the composite part equals **exactly twice** the corresponding lower-bound sieve function throughout the relevant range. This is an identity between the two sieve functions [**P**]; reading it as a loss in a count of pairs needs the sequence, the density and the level of distribution that [P9, §3.1] states are not supplied there. Within that reading, a purely sieve-theoretic attempt loses a clean factor of two, uniformly in every parameter — and [P9, §3.3] locates why Chen's switching principle cannot repair it: it is aimed at the region where the sieve is vacuous, whereas the twin deficit lives where the sieve works.
