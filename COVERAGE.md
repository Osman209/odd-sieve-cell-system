...
|---|---|---|---|---|
| `verify_quadratic_staircase.py` | [P1], all sections | `W_j = ⌊2(j+1)²/n⌋ − ⌊2j²/n⌋` over the first cycle `0 ≤ j ≤ n−1`; local maxima of `2j² mod n` taken **strictly**, with the weaker reading through `ε` checked to agree from `n = 11` on; the exception list of Thm 5 carried by an exhaustive pass over odd `3 ≤ n ≤ 49` | odd `n ≤ 20,001` (`--full`: 200,001) | exact — integer identities, no tolerance |
...
- **[P9, §2.2]'s mean of `(−1)^Ω` over rough `n`.** The `4×10⁷` factorisations are inside
  ...
  not check that row.
| `verify_paired_almost_primes.py` | [P12, Prop. 1], and through it [P12, Thm 1] and [P12, Thm 2] | encloses the dimension-two DHR pair from `σ₂` on `(0,4]`, the integral recursion to `5.360`, and a monotone march of the two delayed equations above `α₂`; then returns a lower bound for `C_r(θ;w,u) = (r+1−u) f₂(θw) − 2∫(1/t − u) F₂(w(θ−t)) dt`. `α₂` is bracketed in `[5.3576,5.3578]` and handled by the fact that `s²F₂(s)` increases throughout, so no decimal place of it is relied on | grid step `0.0002` to `s = 10`; the integral on 20,000 pieces | exact enclosure — outward-rounded Decimal at 34 digits, monotone Riemann bounds, no quadrature rule. Three of the four bounds carry a wide margin; the `theta = 0.513` row is deliberately near its threshold, at about eight enclosure widths |
| `verify_square_window_deficit.py` | [P12, §6] stability, [P12, §9] | solves the dimension-two DHR system in floating point, checks the march against `F, f -> 1`, then reports at `w = 16.9155`, `u = 2.8215`, `theta = 1/2`, `r = 4`: the deficit `C_4`, the split of the subtracted estimate into its floor at `F_2 = 1` and the excess above it, the band table of the weighted excess, the saving `lambda` needed on each tail `s >= S`, the §9.5 sensitivity with `F_2` replaced by 1, the two crossings in the window-length exponent, and the §6 corner check at `(alpha_2, beta_2) = (5.3550, 4.2680)` and `(5.3600, 4.2650)` | one grid of 2,000,001 points on each integration; DHR march at step `2e-4` to `s = 14` | floating point, **not** an enclosure. Only four structural facts are asserted: the deficit is positive, floor plus excess equals the subtracted term, raising `f_2` to its supremum saves under 5% of the deficit, and more than half the excess sits above `s = 4.3` |
