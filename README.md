...
2. **The obstruction stated from inside.** Paper 9 derives the barrier in the construction's
   ...
   crossing.
3. **Negative results and withdrawn claims, kept.** Appendix A of Paper 11 lists results this
...
4. **Runnable code for every measured number.** Every table carrying a measured
   figure has a script in `code/` that regenerates it, and every script exits
   non-zero when its claim fails. Summary tables, proof-case tables and purely
   algebraic tables are not measurements and carry no script; figures quoted from
   the literature carry a citation rather than a script. Three tables are named in
   their own text as regenerated by the original run only: the two transfer rows at
   \(X = 10^{10}\) and the Richert-weight table of Paper 10.
...
changed when the set was split into eleven parts and they stopped sharing one numbering; they
differ from the earlier seven-document releases.
...
`COVERAGE.md` carries one row per script, and the column that matters is not which section
it covers but **the definition it computes**. Three separate review passes each found a
script passing green against a quantity the paper had not stated — a phase normalised one
way in the text and another in the code, a theorem stated for a whole list and tested on a
sub-list, a deviation normalised against the wrong cell count. `audit.py` now refuses a
script with no row and a row with no script; writing the row is what makes the mismatch
visible.
