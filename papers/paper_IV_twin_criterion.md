...
**The lines $5$ and $7$ alone cut six to three, and the pair is not interchangeable.** Since $M = 6n+3$, the phases of $5$ and $7$ on the six positions depend only on $n \bmod 35$, so the question is finite: all $35$ classes can be checked. Doing so gives $\max|S_M \cap E_M| \le 3$ after those two lines alone, with only four maximal patterns —
...
There $S_M \cap E_M = \varnothing$ from the lines $5$ and $7$ alone, so **every** open cell is a twin and the requirement drops from $C_M \ge 7$, or from (3.2), to the bare $C_M \ge 1$. Measured over the $71$ such sectors below $M = 3000$: $7{,}818$ open cells and **not one of them fails to be a twin**, against $155$ non-twin survivors in the $428$ ordinary sectors of the same range. And the restriction costs nothing in the count itself — the survivor density in these five classes, against all other classes, is $0.934$ up to $M = 4000$, $0.997$ up to $12000$ and $1.000$ up to $30000$; fixing the phases of $5$ and $7$ changes which cells die, not how many.
